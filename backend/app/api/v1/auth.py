...
from app.schemas.api_response import APIResponse, CreatedResponse, SuccessResponse
...
from app.schemas.user import UserLogin, UserRegistration, UserWithProfile
...
async def login(
    ...
    try:
        ...
        return APIResponse(success=True, message="Login successful", data=auth_response)
...
async def oauth_login(
    ...
    try:
        ...
        return APIResponse(
            success=True, message="OAuth authentication successful", data=auth_response
        )
...
async def refresh_token(
    ...
    try:
        auth_service = AuthService(
            None, None
        )  # No DB or email service needed for token refresh
        token = await auth_service.refresh_token(current_user)

        logger.info(f"Token refreshed for user: {current_user.email}")

        return APIResponse(
            success=True, message="Token refreshed successfully", data=token
        )
...
async def logout(current_user: User = Depends(get_current_user)) -> SuccessResponse:
    """
    Logout user.
...
async def check_email_availability(
    ...
    try:
        ...
        return APIResponse(
            ...
            data={"email": email, "available": is_available},
        )

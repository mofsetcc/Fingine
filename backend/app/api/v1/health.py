...
async def system_resources_health():
    ...
    try:
        ...
        raise HTTPException(
            status_code=500,
            detail=f"System resources check failed: {str(e)}"
        )
...
async def enable_adapter(
    ...
    try:
        ...
        return {
            "status": "success",
            "message": f"Adapter '{adapter_name}' enabled"
        }
    except HTTPException:
        raise
    except Exception as e:
        raise HTTPException(
            status_code=500,
            detail=f"Failed to enable adapter: {str(e)}"
        )
...
async def disable_adapter(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Failed to disable adapter: {str(e)}"
        )

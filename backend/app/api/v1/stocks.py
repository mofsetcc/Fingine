...
from app.core.deps import get_db, get_current_user_optional, check_api_quota
from app.models.user import User
from app.services.stock_service import StockService
from app.schemas.stock import (
    StockSearchQuery, StockSearchResponse, StockDetail,
    MarketIndex, HotStocksResponse, PriceHistoryRequest, PriceHistoryResponse,
    BatchPriceRequest, BatchPriceResponse
)
...
async def get_price_history(
    ticker: str,
    period: str = Query("1y", description="Time period (1d, 1w, 1m, 3m, 6m, 1y, 2y, 5y)"),
...
async def get_stocks_mentioned_in_news(
    ...
    min_relevance: float = Query(0.2, ge=0.0, le=1.0, description="Minimum relevance score"),
    db: Session = Depends(get_db),
    current_user: Optional[User] = Depends(get_current_user_optional)
):
    """
    ...
    **Parameters:**
    ...
    try:
        ...
        for article in articles:
            ...
            stocks_mentioned = await news_stock_mapping_service.get_stocks_mentioned_in_article(
        ...
        for related_ticker, mentions in related_stocks.items():
            if mentions:  # Only include stocks with mentions
                ...
                avg_relevance = sum(m["relevance_score"] for m in mentions) / len(mentions)
                
                # Get stock details
                stocks_mentioned = await news_stock_mapping_service.get_stocks_mentioned_in_article(
                    mentions[0]["article_id"], min_relevance=0.0
                )
                stock_detail = next((s for s in stocks_mentioned if s["ticker"] == related_ticker), None)
        ...
        stock_stats.sort(key=lambda x: (x["mention_count"], x["avg_relevance_score"]), reverse=True)
...
async def process_news_mapping(
    ...
    batch_size: int = Query(100, ge=10, le=500, description="Batch size for processing"),
    db: Session = Depends(get_db),
    current_user: Optional[User] = Depends(get_current_user_optional)
):
    """

...
async def link_oauth_account(
    ...
    try:
        ...
        existing_identity = db.query(UserOAuthIdentity).filter(
        ...
        logger.info(f"OAuth account linked successfully: {provider} for user {current_user.email}")
...
async def unlink_oauth_account(
    ...
    try:
        ...
        # Find OAuth identity
        oauth_identity = db.query(UserOAuthIdentity).filter(
            UserOAuthIdentity.provider == provider,
        ...
        if not current_user.password_hash:
            ...
            other_identities = db.query(UserOAuthIdentity).filter(
                UserOAuthIdentity.user_id == current_user.id,
                UserOAuthIdentity.provider != provider
            ).count()
        ...
        logger.info(f"OAuth account unlinked successfully: {provider} for user {current_user.email}")
...
async def get_linked_oauth_accounts(
    ...
    try:
        ...
        oauth_identities = db.query(UserOAuthIdentity).filter(
            UserOAuthIdentity.user_id == current_user.id
        ).all()

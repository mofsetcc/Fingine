...
async def create_plan(
    ...
    try:
        ...
        return APIResponse(success=True, message="Plan created successfully", data=plan)
    ...
    try:
        ...
        if current_plan_id:
            ...
            if current_plan:
                ...
                higher_plans = [
                    p for p in plans if p.price_monthly > current_plan.price_monthly
                ]
                if higher_plans:
                    recommended_plan_id = min(
                        higher_plans, key=lambda p: p.price_monthly
                    ).id
        else:
...
async def update_plan(
    ...
    try:
        ...
        return APIResponse(success=True, message="Plan updated successfully", data=plan)
...
async def get_usage_quota(
    ...
    try:
        ...
        return APIResponse(
            success=True, message="Usage quota retrieved successfully", data=usage_quota
        )
...
async def downgrade_subscription(
    ...
    try:
        service = SubscriptionService(db)
        subscription = await service.downgrade_subscription(
            current_user.id, downgrade_data
        )
        ...
        logger.error(
            f"Failed to downgrade subscription for user {current_user.id}: {e}"
        )
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
...
async def initialize_default_plans(
    ...
    try:
        ...
        return APIResponse(
            success=True, message="Default plans initialized successfully", data=plans
        )
...
async def check_quota_availability(
    ...
    try:
        ...
        return APIResponse(
            ...
            data=QuotaCheckResult(has_quota=has_quota, quota_info=quota_info),
        )

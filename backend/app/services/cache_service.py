...
from app.core.cache import cache_manager, CacheKeyType, CacheKeyBuilder
...
class CacheService:
    ...
    async def get_or_set(
        ...
        try:
            ...
            if data is not None:
                await self.cache_manager.cache.set(key, data, ttl=ttl, key_type=key_type)
    ...
    async def track_api_quota(self, user_id: str, quota_type: str = "daily") -> int:
        ...
        try:
            ...
            logger.debug("API quota tracked", user_id=user_id, quota_type=quota_type, usage=current_usage)
    ...
    async def invalidate_user_session(self, user_id: str) -> bool:
        ...
        try:
            ...
            logger.error("Failed to invalidate user session", user_id=user_id, error=str(e))
            return False

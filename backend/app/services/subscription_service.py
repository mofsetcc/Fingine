...
from app.schemas.subscription import (
    ...
    UsageQuota, SubscriptionWithUsage
)
...
class SubscriptionService:
    ...
    async def create_subscription(self, subscription_data: SubscriptionCreate) -> Subscription:
        ...
        try:
            ...
            logger.info(f"Created subscription for user {subscription_data.user_id} with plan {subscription_data.plan_id}")
            return subscription
    ...
    async def upgrade_subscription(self, user_id: UUID, upgrade_data: SubscriptionUpgrade) -> Subscription:
        ...
        try:
            ...
            logger.info(f"Upgraded subscription for user {user_id} to plan {upgrade_data.new_plan_id}")
            return subscription
    ...
    async def downgrade_subscription(self, user_id: UUID, downgrade_data: SubscriptionDowngrade) -> Subscription:
        ...
        try:
            ...
            effective_date = downgrade_data.effective_date or datetime.fromisoformat(subscription.current_period_end)
            ...
            if effective_date <= datetime.utcnow():
                ...
                logger.info(f"Scheduled downgrade for user {user_id} effective {effective_date}")
            
            self.db.commit()
            self.db.refresh(subscription)
            
            logger.info(f"Downgraded subscription for user {user_id} to plan {downgrade_data.new_plan_id}")
            return subscription
    ...
    async def cancel_subscription(self, user_id: UUID, cancel_data: SubscriptionCancel) -> Subscription:
        ...
        try:
            ...
            logger.info(f"Cancelled subscription for user {user_id}, reason: {cancel_data.reason}")
            return subscription
    ...
    # Utility Methods
    async def get_subscription_with_usage(self, user_id: UUID) -> Optional[SubscriptionWithUsage]:

        ...
        try:
            ...
            async with httpx.AsyncClient() as client:
                ...
                if token_info_response.status_code != 200:
                    logger.warning(f"Google token verification failed: {token_info_response.status_code}")
                ...
                if userinfo_response.status_code != 200:
                    logger.error(f"Failed to get Google user info: {userinfo_response.status_code}")
    ...
    async def exchange_code_for_token(self, code: str) -> Dict[str, Any]:
        ...
        try:
            async with httpx.AsyncClient() as client:
                ...
                if response.status_code != 200:
                    logger.error(f"LINE token exchange failed: {response.status_code} - {response.text}")
                    raise ProviderError("Failed to exchange code for token")
        ...
        try:
            async with httpx.AsyncClient() as client:
                ...
                if verify_response.status_code != 200:
                    logger.warning(f"LINE token verification failed: {verify_response.status_code}")
                    raise InvalidTokenError("Invalid access token")
                ...
                if profile_response.status_code != 200:
                    logger.error(f"Failed to get LINE user profile: {profile_response.status_code}")
                    raise ProviderError("Failed to get user information")
...
class OAuthService:
    ...
    async def verify_token(self, provider_name: str, access_token: str) -> Dict[str, Any]:
        """
    ...
    async def refresh_token(self, provider_name: str, refresh_token: str) -> Dict[str, Any]:
        """

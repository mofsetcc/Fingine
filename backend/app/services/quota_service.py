...
class QuotaService:
    ...
    async def get_user_quotas(self, user_id: UUID) -> Dict[str, int]:
        ...
        if not subscription or subscription.status != "active":
            ...
            return {"api_quota_daily": 10, "ai_analysis_quota_daily": 5}
    ...
    async def get_user_usage_today(self, user_id: UUID) -> Dict[str, int]:
        ...
        api_usage_count = (
            ...
            .filter(
                and_(
                    ...
                    func.to_timestamp(
                        APIUsageLog.request_timestamp, 'YYYY-MM-DD"T"HH24:MI:SS.US"Z"'
                    )
                    >= today_start,
                    func.to_timestamp(
                        APIUsageLog.request_timestamp, 'YYYY-MM-DD"T"HH24:MI:SS.US"Z"'
        ...
        ai_analysis_count = (
            self.db.query(func.count(AIAnalysisCache.ticker))
            .filter(
                and_(
            ...
            .join(
                ...
                and_(
                    func.date(
                        func.to_timestamp(
                            APIUsageLog.request_timestamp,
                            'YYYY-MM-DD"T"HH24:MI:SS.US"Z"',
                        )
                    )
                    == func.date(AIAnalysisCache.created_at),
                    APIUsageLog.request_type == "ai_analysis",
                ),
            )
            .scalar()
    ...
    async def check_quota_available(
        ...
        if quota_type == "api":
            ...
            has_quota = (
                usage_quota.ai_analysis_usage_today
                < usage_quota.ai_analysis_quota_daily
            )
            quota_info = {
                "quota_type": "ai_analysis",
                "usage": usage_quota.ai_analysis_usage_today,
                "limit": usage_quota.ai_analysis_quota_daily,
                "remaining": usage_quota.ai_analysis_quota_daily
                - usage_quota.ai_analysis_usage_today,
                "reset_at": usage_quota.quota_reset_at.isoformat(),
            }
        else:
    ...
    async def record_api_usage(
        ...
        try:
            ...
            logger.debug(
                f"Recorded API usage for user {user_id}: {api_provider}/{endpoint}"
            )
            return usage_log
    ...
    async def get_quota_usage_summary(
        ...
        return {
            ...
            "total_ai_analysis_calls": sum(
                day["ai_analysis_calls"] for day in usage_by_date
            ),

...
class DataSourceService:
    ...
    async def get_historical_prices(
        ...
        try:
            async def operation(adapter):
                return await adapter.get_historical_prices(symbol, start_date, end_date, interval)
            ...
            logger.info(f"Retrieved historical prices for {symbol} from {start_date} to {end_date}")
            return result
    ...
    async def get_financial_statements(
        ...
        try:
            async def operation(adapter):
                return await adapter.get_financial_statements(symbol, statement_type, period)
    ...
    async def get_news(
        ...
        try:
            async def operation(adapter):
                return await adapter.get_news(symbol, keywords, limit, start_date, end_date)
    ...
    async def get_data_source_status(self) -> Dict[str, Any]:
        ...
        try:
            ...
            healthy_adapters = sum(
                1 for adapter in status["adapters"].values()
    ...
    async def reset_adapter_circuit_breaker(self, adapter_name: str) -> bool:
        ...
        try:
            ...
            if result:
                ...
                logger.warning(f"Circuit breaker was not open for adapter: {adapter_name}")

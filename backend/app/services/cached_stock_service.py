...
from app.schemas.stock import (
    StockSearchQuery, StockSearchResult, StockSearchResponse,
...
class CachedStockService:
    ...
    async def search_stocks_cached(self, query: StockSearchQuery) -> StockSearchResponse:
        """
    ...
    async def _search_stocks_uncached(self, query: StockSearchQuery) -> StockSearchResponse:
        """
        ...
        if search_term.isdigit() and len(search_term) <= 4:
            search_conditions.append(
                (Stock.ticker == search_term.zfill(4), 1.0)
            )
        
        # Ticker prefix match
        if search_term.isdigit():
            search_conditions.append(
                (Stock.ticker.like(f"{search_term}%"), 0.9)
            )
        ...
        search_query = (
            self.db.query(
                Stock,
                relevance_expr.label('match_score')
            )
        ...
        for result in results:
            ...
            search_results.append(StockSearchResult(
                ticker=stock.ticker,
        ...
        total_query = (
            self.db.query(func.count(Stock.ticker))
            .filter(or_(*combined_conditions))
        )
    ...
    async def _get_latest_prices_cached(self, tickers: List[str]) -> Dict[str, Dict[str, Any]]:
        """
        ...
        for ticker in tickers:
            cached_price = await cache_service.get_stock_price_cached(
                ticker,
                lambda: self._fetch_single_price(ticker)
            )
    ...
    async def _fetch_multiple_prices(self, tickers: List[str]) -> Dict[str, Dict[str, Any]]:
    ...
    async def get_stock_detail_cached(self, ticker: str) -> Optional[StockDetail]:
        ...
        return await cache_service.get_stock_price_cached(
            ticker,
            fetch_stock_detail
        )
    ...
    async def _get_stock_detail_uncached(self, ticker: str) -> Optional[StockDetail]:
        ...
        return StockDetail(
            ...
            pe_ratio=float(latest_metrics.pe_ratio) if latest_metrics and latest_metrics.pe_ratio else None,
            pb_ratio=float(latest_metrics.pb_ratio) if latest_metrics and latest_metrics.pb_ratio else None,
            dividend_yield=float(latest_metrics.dividend_yield) if latest_metrics and latest_metrics.dividend_yield else None
        )
    ...
    async def get_hot_stocks_cached(self) -> HotStocksResponse:
        ...
        return await cache_service.get_or_set(
            cache_key,
            fetch_hot_stocks,
            key_type=CacheKeyType.MARKET_DATA
        )

...
class CostManager:
    ...
    async def can_afford(self, estimated_cost: float) -> bool:
        ...
        try:
            ...
            if (
                remaining_daily < self.emergency_buffer
                and estimated_cost > remaining_daily * 0.5
            ):
                logger.warning(
    ...
    def _get_cache_threshold(
        self, current_time: datetime, analysis_cost: Optional[float] = None
    ) -> int:
    ...
    async def _get_daily_usage(self) -> float:
        ...
        try:
            today = date.today()
            result = (
                self.db.query(func.sum(APIUsageLog.cost_usd))
                .filter(
    ...
    def estimate_analysis_cost(
        self, analysis_type: str, data_complexity: str = "medium"
    ) -> float:
...
class IntelligentCacheManager:
    ...
    def get_cache_ttl(
        self, data_type: str, market_conditions: Dict[str, Any] = None
    ) -> int:
    ...
    def should_invalidate_cache(
        self, data_type: str, last_update: datetime, trigger_events: List[str] = None
    ) -> bool:

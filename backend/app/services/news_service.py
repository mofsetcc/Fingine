...
class NewsCollectionService:
    ...
    def _initialize_adapter(self) -> None:
        ...
        try:
            config = {"news_api_key": settings.NEWS_API_KEY}

            self.news_adapter = NewsDataAdapter(
                name="primary_news_adapter", priority=1, config=config
            )
    ...
    async def collect_general_news(self) -> int:
        ...
        try:
            ...
            keywords = ["日本株式市場", "東京証券取引所", "日経平均", "TOPIX", "株価", "決算", "業績", "投資"]
    ...
    async def _store_articles(
        self, articles: List[Dict[str, Any]], ticker: Optional[str] = None
    ) -> int:
        """
            ...
            async with get_db() as db:
                for article_data in articles:
                    ...
                    if article_url:
                        existing = await db.execute(
                            select(NewsArticle).where(
                                NewsArticle.article_url == article_url
                            )
                        )
                    ...
                    try:
                        ...
                        logger.error(
                            f"Error creating stock-news links for article {article.id}: {e}"
                        )
                        ...
                        if stock:
                            ...
                            if existing_link:
                                ...
                                stock_link = StockNewsLink(
                                    ...
                                    ticker=ticker,
                                    relevance_score=article_data.get(
                                        "relevance_score", 0.8
                                    ),
                                )
                                db.add(stock_link)
    ...
    async def cleanup_old_articles(self) -> int:
        ...
        try:
            cutoff_date = datetime.utcnow() - timedelta(
                days=self.article_retention_days
            )

            async with get_db() as db:
                # Find old articles
                result = await db.execute(
                    select(NewsArticle).where(NewsArticle.created_at < cutoff_date)
                )
    ...
    async def process_existing_articles_for_mapping(
        self, batch_size: int = 100, force_refresh: bool = False
    ) -> Dict[str, int]:
        """
        ...
        try:
            async with get_db() as db:
                ...
                stats = (
                    await news_stock_mapping_service.batch_create_links_for_articles(
        ...
        try:
            async with get_db() as db:
                ...
                recent_result = await db.execute(
                    select(NewsArticle).where(NewsArticle.created_at >= recent_cutoff)
                )

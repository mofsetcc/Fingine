...
class DataTransformer:
    ...
    async def prepare_analysis_context(
        ...
        try:
            ...
            context["analysis_summary"] = self._generate_analysis_summary(
    ...
    async def _prepare_technical_context(self, ticker: str) -> Dict[str, Any]:
        ...
        try:
            ...
            support_resistance = self.indicator_calc.calculate_support_resistance(
                closes
            )
    ...
    async def _get_financial_data(self, ticker: str) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            reports = (
                ...
                .filter(FinancialReport.ticker == ticker)
                .order_by(
            ...
            for report in reports:
                ...
                line_items = (
                    self.db.query(FinancialReportLineItem)
                    .filter(FinancialReportLineItem.report_id == report.id)
                    .all()
                )

                metrics = {
                    item.metric_name: float(item.metric_value) for item in line_items
                }
    ...
    async def _get_daily_metrics(self, ticker: str) -> Dict[str, Any]:
        ...
        try:
            latest_metrics = (
                ...
                .order_by(StockDailyMetrics.date.desc())
                .first()
            )
            ...
            return {
                ...
                "market_cap": int(latest_metrics.market_cap)
                ...
                if latest_metrics.dividend_yield
                else None,
                "shares_outstanding": int(latest_metrics.shares_outstanding)
                if latest_metrics.shares_outstanding
                else None,
            }
    ...
    async def _get_news_data(
        ...
        try:
            ...
            news_query = (
                ...
                .join(StockNewsLink, NewsArticle.id == StockNewsLink.article_id)
                .filter(
    ...
    def _format_price_data(self, price_data: List[Dict[str, Any]]) -> str:
        ...
        for data in price_data[-5:]:  # Last 5 days
            formatted += (
                f"日付: {data['date']}, 終値: ¥{data['close']}, 出来高: {data['volume']:,}\n"
            )
    ...
    def _analyze_quarterly_trends(self, financial_data: List[Dict[str, Any]]) -> str:
        ...
        quarterly_data = [
            d for d in financial_data if d.get("report_type") == "quarterly"
        ]
    ...
    def _format_recent_news(self, news_data: List[Dict[str, Any]]) -> str:
        ...
        for news in news_data:
            sentiment_emoji = (
                "📈"
                if news.get("sentiment_label") == "positive"
                else "📉"
        ...
        try:
            ...
            return {
                "momentum_analysis": {
                    ...
                    "momentum_summary": self._analyze_momentum_trend(
        ...
        try:
            ...
            return {
                "growth_analysis": {
                    ...
                    "growth_consistency": growth_metrics.get(
                        "growth_consistency", "不明"
                    ),
    ...
    def _analyze_momentum_trend(
        self, momentum_5: List[float], momentum_10: List[float]
    ) -> str:
    ...
    def _calculate_valuation_metrics(
        self, daily_metrics: Dict[str, Any], financial_data: List[Dict[str, Any]]
    ) -> Dict[str, Any]:
        """Calculate valuation metrics."""
        try:
    ...
    def _generate_analysis_summary(
        ...
        try:
            ...
            if "news_sentiment" in context and isinstance(
                context["news_sentiment"], str
            ):
                summary += "【ニュース・センチメント】\n"

...
class NewsStockMappingService:
    ...
    async def refresh_stock_cache(self) -> None:
        ...
        try:
            async with get_db() as db:
                ...
                logger.info(f"Refreshed stock cache with {len(self._stock_cache)} active stocks")
    ...
    async def _ensure_stock_cache(self) -> None:
        ...
        if (self._cache_last_updated is None or 
            (datetime.utcnow() - self._cache_last_updated).total_seconds() > self._cache_ttl_seconds):
            ...
            async with get_db() as db:
                ...
                if not force_refresh:
                    existing_links = await db.execute(
                        select(StockNewsLink).where(StockNewsLink.article_id == article_id)
                    )
                ...
                for ticker, relevance_score in stock_matches:
                    if relevance_score >= self.min_relevance_threshold:
                        ...
                        created_links.append({
                            "ticker": ticker,
                            "relevance_score": relevance_score,
                            "company_name": self._stock_cache[ticker].company_name_jp
                        })
                
                await db.commit()
                
                logger.info(f"Created {len(created_links)} stock links for article {article_id}")
                return created_links
                
        except Exception as e:
            logger.error(f"Error creating stock-news links for article {article_id}: {e}")
            return []
    ...
    def _calculate_relevance_score(
        ...
        if score > 0:
            financial_keyword_count = sum(
                1 for keyword in self._financial_keywords
    ...
    async def batch_create_links_for_articles(
        ...
        try:
            ...
            for i in range(0, len(article_ids), batch_size):
                ...
                logger.info(f"Processed batch {i//batch_size + 1}: {len(batch_ids)} articles")
    ...
    async def get_news_for_stock(
        ...
        try:
            async with get_db() as db:
                ...
                if start_date:
                    query = query.where(NewsArticle.published_at >= start_date.isoformat())
                if end_date:
                    query = query.where(NewsArticle.published_at <= end_date.isoformat())
            ...
            async with get_db() as db:
                ...
                for link in article.stock_links:
                    if link.ticker in self._stock_cache:
                        ...
                        if force_recalculate or abs(float(link.relevance_score) - new_score) > 0.1:
                            link.relevance_score = new_score
                            updated_count += 1
                
                await db.commit()
                
                logger.info(f"Updated {updated_count} relevance scores for article {article_id}")
                return updated_count
                
        except Exception as e:
            logger.error(f"Error updating relevance scores for article {article_id}: {e}")
            return 0
        ...
        try:
            async with get_db() as db:
                ...
                return {
                    ...
                    "linking_rate": linked_articles / total_articles if total_articles > 0 else 0,
                    "total_links": total_links,
                    "avg_links_per_article": total_links / linked_articles if linked_articles > 0 else 0,

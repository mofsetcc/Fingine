...
class AuthService:
    ...
    async def register_user(self, user_data: UserRegistration) -> UserSchema:
        ...
        if existing_user:
            logger.warning(
                f"Registration attempt for existing email: {user_data.email}"
            )
    ...
    async def oauth_authenticate(self, oauth_data: OAuthLogin) -> AuthResponse:
        ...
        """
        logger.info(
            f"OAuth authentication attempt with provider: {oauth_data.provider}"
        )

        # Verify OAuth token and get user info
        user_info = await self._verify_oauth_token(
            oauth_data.provider, oauth_data.access_token
        )
        if not user_info:
            logger.warning(
                f"OAuth authentication failed - invalid token: {oauth_data.provider}"
            )
    ...
    async def request_password_reset(self, email_data: PasswordReset) -> bool:
        ...
        if user:
            ...
            logger.warning(
                f"Password reset requested for non-existent user: {email_data.email}"
            )
    ...
    async def resend_verification_email(self, email: str) -> bool:
        ...
        if user and not user.email_verified_at:
            ...
            logger.warning(
                f"Resend verification requested for verified or non-existent user: {email}"
            )

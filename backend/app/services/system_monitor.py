...
class SystemMonitor:
    ...
    def __init__(self):
        ...
        self._service_metrics: Dict[str, deque] = defaultdict(lambda: deque(maxlen=self._max_history_points))
        ...
        try:
            ...
            for service_name, service_data in health_data.get("services", {}).items():
                ...
                if service_name == "system_resources":
                    ...
                    service_metric.update({
                        "healthy_count": service_data.get("summary", {}).get("healthy", 0),
                        "total_count": service_data.get("summary", {}).get("total", 0)
                    })
    ...
    async def _check_alerts(self, health_data: Dict[str, Any], timestamp: datetime) -> None:
        ...
        if resources.get("memory_percent", 0) > self._alert_thresholds["memory_percent"]:
            alerts.append({
                "type": "high_memory",
                "severity": "warning",
    ...
    def _get_history_summary(self) -> Dict[str, Any]:
        ...
        healthy_points = sum(1 for point in history_list if point["status"] == ServiceStatus.HEALTHY.value)
        uptime_percentage = (healthy_points / total_points * 100) if total_points > 0 else 0
        ...
        response_times = [point["duration_ms"] for point in history_list if point.get("duration_ms")]
    ...
    async def get_service_metrics(self, service_name: str, hours: int = 1) -> Dict[str, Any]:
        ...
        filtered_metrics = [
            metric for metric in service_history
        ...
        response_times = [m["response_time_ms"] for m in filtered_metrics if m.get("response_time_ms")]
        error_count = sum(1 for m in filtered_metrics if m.get("error"))
        
        summary = {
            "total_points": len(filtered_metrics),
            "error_count": error_count,
            "error_rate_percent": (error_count / len(filtered_metrics) * 100) if filtered_metrics else 0,

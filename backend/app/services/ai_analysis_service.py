...
class GeminiAnalysisClient:
    ...
    def estimate_cost(self, prompt_length: int, expected_response_length: int = 2000) -> float:
    ...
    def _parse_response(self, response_text: str, analysis_type: str) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.warning(f"Could not parse JSON response for {analysis_type}, returning text")
            return {
            ...
            raise ResponseParsingException(f"Failed to parse analysis response: {str(e)}")
    ...
    def _validate_analysis_result(self, result: Dict[str, Any], analysis_type: str):
        ...
        required_fields = {
            ...
            "comprehensive": ["rating", "confidence", "key_factors", "price_target_range"]
        }
        ...
        if analysis_type in required_fields:
            ...
            if missing_fields:
                logger.warning(f"Missing fields in {analysis_type} analysis: {missing_fields}")
...
class AIAnalysisService:
    ...
    async def generate_analysis(
        ...
        try:
            ...
            logger.info(f"Successfully generated analysis for {ticker} - {analysis_type}")
    ...
    async def _enhance_analysis_result(
        ...
        enhanced_result["analysis_metadata"] = {
            ...
            "data_quality_score": context.get("data_quality", {}).get("overall_score", 0.0),
        ...
        enhanced_result["confidence_metrics"] = await self._calculate_confidence_metrics(
            result, context
        )
    ...
    async def _check_cache(
        ...
        return self.db.query(AIAnalysisCache).filter(
    ...
    def _format_cached_result(self, cached_analysis: AIAnalysisCache) -> Dict[str, Any]:
        ...
        return {
            ...
            "from_cache": True,
            "cache_age_seconds": (datetime.now() - cached_analysis.created_at).total_seconds()
        }
    ...
    async def _get_quarterly_context(self, ticker: str) -> Dict[str, Any]:
        ...
        try:
            quarterly_context = {
                ...
                "quarterly_trends": await self._get_quarterly_performance_trends(ticker),
    ...
    async def _get_annual_context(self, ticker: str) -> Dict[str, Any]:
        ...
        try:
            annual_context = {
                ...
                "capital_allocation": await self._get_capital_allocation_history(ticker)
            }
    ...
    async def _get_strategic_context(self, ticker: str) -> Dict[str, Any]:
        ...
        try:
            strategic_context = {
                ...
                "competitive_advantages": await self._identify_competitive_advantages(ticker),
    ...
    async def _assess_data_quality(self, context: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            available_fields = sum(1 for field in required_fields if field in context and context[field])
        ...
        try:
            ...
            if result.get("from_cache"):
                ...
                if cache_age > 3600:  # 1 hour
                    risk_assessment["data_risks"].append("Analysis based on cached data older than 1 hour")
            
            # Model risks
            confidence = result.get("result", {}).get("confidence", 0.0)
            if confidence < 0.6:
                risk_assessment["model_risks"].append("Low model confidence in analysis")
            
            # Market risks (placeholder)
            risk_assessment["market_risks"].append("General market volatility risk")
            
            # Calculate overall risk level
            total_risks = len(risk_assessment["data_risks"]) + len(risk_assessment["model_risks"]) + len(risk_assessment["market_risks"])
            if total_risks <= 1:

...
class UserService:
    ...
    def get_user_oauth_identities(self, user_id: UUID) -> List[dict]:
        ...
        oauth_identities = self.db.query(UserOAuthIdentity).filter(
    ...
    async def export_user_data(self, user_id: UUID) -> dict:
        ...
        user_data = {
            ...
            "profile": {
                ...
                "notification_preferences": user.profile.notification_preferences if user.profile else {}
            },
    ...
    async def _send_password_set_notification(self, user: User) -> None:
        ...
        try:
            ...
            logger.error(f"Failed to send password set notification to {user.email}: {e}")
    
    async def _send_email_changed_notification(self, old_email: str, new_email: str) -> None:
        ...
        try:
            ...
            logger.error(f"Failed to send email changed notification to {old_email}: {e}")
    
    async def _send_account_deleted_notification(self, email: str, display_name: Optional[str]) -> None:
        """Send account deletion confirmation email."""
        try:
            await self.email_service.send_subscription_notification(
                email=email,

...
class FinancialReport(Base, UUIDMixin, TimestampMixin):
    ...
    __table_args__ = (
        ...
        CheckConstraint(
            "report_type IN ('quarterly', 'annual')", name="check_report_type"
        ),
    ...
    line_items = relationship(
        "FinancialReportLineItem", back_populates="report", cascade="all, delete-orphan"
    )
...
class FinancialReportLineItem(Base):
    ...
    report_id = Column(
        UUID(as_uuid=True), ForeignKey("financial_reports.id"), primary_key=True
    )
    ...
    __table_args__ = (
        CheckConstraint(
            "period_type IN ('quarterly', 'annual', 'ytd')", name="check_period_type"
        ),
    )

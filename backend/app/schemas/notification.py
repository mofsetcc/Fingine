...
class NotificationCreate(NotificationBase):
    ...
    scheduled_for: Optional[datetime] = Field(
        None, description="Schedule notification for later"
    )
...
class NotificationUpdate(BaseModel):
    ...
    is_archived: Optional[bool] = Field(
        None, description="Archive/unarchive notification"
    )
...
class NotificationPreferencesBase(BaseModel):
    ...
    earnings_announcements: bool = Field(
        True, description="Earnings announcement notifications"
    )
    news_alerts: bool = Field(True, description="News alert notifications")
    ai_analysis_complete: bool = Field(
        True, description="AI analysis completion notifications"
    )
    system_maintenance: bool = Field(
        True, description="System maintenance notifications"
    )
    account_updates: bool = Field(True, description="Account update notifications")
    subscription_updates: bool = Field(
        True, description="Subscription update notifications"
    )
    ...
    max_notifications_per_hour: int = Field(
        10, ge=1, le=100, description="Max notifications per hour"
    )
...
class NotificationPreferencesUpdate(BaseModel):
    ...
    volume_alerts: Optional[bool] = Field(
        None, description="Volume alert notifications"
    )
    earnings_announcements: Optional[bool] = Field(
        None, description="Earnings announcement notifications"
    )
    news_alerts: Optional[bool] = Field(None, description="News alert notifications")
    ai_analysis_complete: Optional[bool] = Field(
        None, description="AI analysis completion notifications"
    )
    system_maintenance: Optional[bool] = Field(
        None, description="System maintenance notifications"
    )
    account_updates: Optional[bool] = Field(
        None, description="Account update notifications"
    )
    subscription_updates: Optional[bool] = Field(
        None, description="Subscription update notifications"
    )
    watchlist_updates: Optional[bool] = Field(
        None, description="Watchlist update notifications"
    )
    market_updates: Optional[bool] = Field(
        None, description="Market update notifications"
    )
    ...
    max_notifications_per_hour: Optional[int] = Field(
        None, ge=1, le=100, description="Max notifications per hour"
    )
...
class NotificationTemplateBase(BaseModel):
    ...
    title_template: str = Field(
        ..., max_length=200, description="Title template with placeholders"
    )
    message_template: str = Field(
        ..., max_length=1000, description="Message template with placeholders"
    )
...
class NotificationTemplateUpdate(BaseModel):
    ...
    title_template: Optional[str] = Field(
        None, max_length=200, description="Title template"
    )
    message_template: Optional[str] = Field(
        None, max_length=1000, description="Message template"
    )
...
class NotificationChannelBase(BaseModel):
    ...
    def validate_channel_type(cls, v):
        ...
        if v not in ["email", "push", "sms", "webhook", "slack"]:
            raise ValueError(
                "Channel type must be one of: email, push, sms, webhook, slack"
            )
        return v
...
class NotificationChannelUpdate(BaseModel):
    ...
    configuration: Optional[Dict[str, Any]] = Field(
        None, description="Channel configuration"
    )
...
class NotificationChannel(UUIDSchema, NotificationChannelBase, TimestampSchema):
    ...
    last_used_at: Optional[datetime] = Field(
        None, description="Last time channel was used"
    )
...
class NotificationStatistics(BaseModel):
    ...
    notifications_this_week: int = Field(
        ..., description="Notifications received this week"
    )
    ...
    average_read_time: Optional[float] = Field(
        None, description="Average time to read (minutes)"
    )
    most_active_hours: List[int] = Field(
        ..., description="Hours with most notifications"
    )

...
class UserProfileBase(BaseModel):
    ...
    avatar_url: Optional[str] = Field(
        None, max_length=255, description="Avatar image URL"
    )
    timezone: str = Field(
        default="Asia/Tokyo", max_length=50, description="User timezone"
    )
...
class UserWithProfile(User):
    ...
    oauth_identities: list[UserOAuthIdentity] = Field(
        default_factory=list, description="OAuth identities"
    )
...
class EmailChange(BaseModel):
    ...
    password: Optional[str] = Field(
        None, description="Current password (if user has one)"
    )
...
class UserPreferencesUpdate(BaseModel):
    ...
    privacy_preferences: Optional[Dict[str, Any]] = Field(
...
class GDPRDataRequest(BaseModel):
    ...
    keep_legal_basis: bool = Field(
        True, description="Keep data required for legal compliance"
    )

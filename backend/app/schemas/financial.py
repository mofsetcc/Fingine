...
class FinancialReportBase(BaseModel):
    ...
    def validate_fiscal_period(cls, v):
        ...
        if v not in valid_periods:
            raise ValueError(
                f'Fiscal period must be one of: {", ".join(valid_periods)}'
            )
        return v
...
class BalanceSheetBase(BaseModel):
    ...
    property_plant_equipment: Optional[Decimal] = Field(
        None, description="Property, plant & equipment"
    )
...
class IncomeStatementBase(BaseModel):
    ...
    operating_expenses: Optional[Decimal] = Field(
        None, description="Operating expenses"
    )
    ...
    income_tax_expense: Optional[Decimal] = Field(
        None, description="Income tax expense"
    )
    net_income: Optional[Decimal] = Field(None, description="Net income")
    ...
    weighted_average_shares: Optional[int] = Field(
        None, description="Weighted average shares"
    )
...
class CashFlowBase(BaseModel):
    ...
    depreciation_amortization: Optional[Decimal] = Field(
        None, description="Depreciation & amortization"
    )
    changes_working_capital: Optional[Decimal] = Field(
        None, description="Changes in working capital"
    )
...
class FinancialRatiosBase(BaseModel):
    ...
    receivables_turnover: Optional[float] = Field(
        None, description="Receivables turnover"
    )
...
class FinancialSummary(BaseModel):
    ...
    latest_total_assets: Optional[Decimal] = Field(
        None, description="Latest total assets"
    )
    latest_total_equity: Optional[Decimal] = Field(
        None, description="Latest total equity"
    )
...
class EarningsCalendarEntry(BaseModel):
    ...
    def validate_market_time(cls, v):
        ...
        if v not in [
            ...
            "AMC",
            "DMT",
        ]:  # Before Market Open, After Market Close, During Market Time

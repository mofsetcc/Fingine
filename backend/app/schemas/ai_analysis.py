...
class AIAnalysisRequest(BaseModel):
    ...
    def validate_analysis_type(cls, v):
        ...
        valid_types = [
            'fundamental', 'technical', 'sentiment', 'risk_assessment',
            'price_prediction', 'earnings_forecast', 'peer_comparison',
            'market_outlook', 'investment_recommendation'
        ]
...
class BulkAnalysisRequest(BaseModel):
    ...
    tickers: List[str] = Field(..., min_items=1, max_items=50, description="Stock tickers to analyze")
    analysis_type: str = Field(..., description="Type of analysis requested")
    parameters: Optional[Dict[str, Any]] = Field(None, description="Analysis parameters")
    language: str = Field("ja", description="Response language (ja, en)")
...
class FundamentalAnalysisResult(BaseModel):
    ...
    overall_score: float = Field(..., ge=0, le=100, description="Overall fundamental score")
    financial_health: Dict[str, Any] = Field(..., description="Financial health metrics")
    ...
    competitive_position: Dict[str, Any] = Field(..., description="Competitive analysis")
...
class TechnicalAnalysisResult(BaseModel):
    ...
    support_resistance: Dict[str, Any] = Field(..., description="Support/resistance levels")
    ...
    def validate_signal(cls, v):
        ...
        if v not in ['strong_buy', 'buy', 'hold', 'sell', 'strong_sell']:
            raise ValueError('Signal must be one of: strong_buy, buy, hold, sell, strong_sell')
        return v
...
class SentimentAnalysisResult(BaseModel):
    ...
    sentiment_trends: List[Dict[str, Any]] = Field(..., description="Sentiment trends over time")
    ...
    def validate_sentiment(cls, v):
        ...
        if v not in ['very_positive', 'positive', 'neutral', 'negative', 'very_negative']:
            raise ValueError('Sentiment must be one of: very_positive, positive, neutral, negative, very_negative')
        return v
...
class RiskAssessmentResult(BaseModel):
    ...
    def validate_risk_level(cls, v):
        ...
        if v not in ['very_low', 'low', 'moderate', 'high', 'very_high']:
            raise ValueError('Risk level must be one of: very_low, low, moderate, high, very_high')
        return v
...
class PricePredictionResult(BaseModel):
    ...
    scenarios: Dict[str, Dict[str, float]] = Field(..., description="Different scenarios")
    ...
    def validate_horizon(cls, v):
        ...
        if v not in ['1_week', '1_month', '3_months', '6_months', '1_year']:
            raise ValueError('Horizon must be one of: 1_week, 1_month, 3_months, 6_months, 1_year')
        return v
...
class AnalysisHistory(BaseModel):
    ...
    favorite_analysis_types: List[str] = Field(..., description="Most used analysis types")
...
class AnalysisExportRequest(BaseModel):
    ...
    analysis_ids: List[UUID] = Field(..., min_items=1, max_items=100, description="Analysis IDs to export")
...
class AnalysisExportResponse(BaseModel):
    ...
    def validate_status(cls, v):
        ...
        if v not in ['pending', 'processing', 'completed', 'failed', 'expired']:
            raise ValueError('Status must be one of: pending, processing, completed, failed, expired')
        return v
...
class AnalysisStatistics(BaseModel):
    ...
    most_analyzed_stocks: List[Dict[str, Any]] = Field(..., description="Most analyzed stocks")

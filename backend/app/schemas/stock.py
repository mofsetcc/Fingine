...
class StockBase(BaseModel):
    ...
    company_name_jp: str = Field(
        ..., max_length=255, description="Japanese company name"
    )
    company_name_en: Optional[str] = Field(
        None, max_length=255, description="English company name"
    )
    sector_jp: Optional[str] = Field(
        None, max_length=100, description="Japanese sector name"
    )
    industry_jp: Optional[str] = Field(
        None, max_length=100, description="Japanese industry name"
    )
    description: Optional[str] = Field(None, description="Company description")
    logo_url: Optional[str] = Field(
        None, max_length=255, description="Company logo URL"
    )
...
class StockUpdate(BaseModel):
    ...
    company_name_jp: Optional[str] = Field(
        None, max_length=255, description="Japanese company name"
    )
    company_name_en: Optional[str] = Field(
        None, max_length=255, description="English company name"
    )
    ...
    is_active: Optional[bool] = Field(
        None, description="Whether stock is actively traded"
    )
...
class StockDailyMetricsBase(BaseModel):
    ...
    shares_outstanding: Optional[int] = Field(
        None, ge=0, description="Shares outstanding"
    )
...
class TechnicalIndicators(BaseModel):
    ...
    moving_avg_200: Optional[Decimal] = Field(
...
class BatchPriceRequest(BaseModel):
    ...
    tickers: List[str] = Field(
        ..., min_items=1, max_items=50, description="Stock tickers"
    )
    ...
    def validate_tickers(cls, v):
        ...
        for ticker in v:
            if not ticker.isdigit() or len(ticker) != 4:
                raise ValueError(
                    f"Invalid ticker format: {ticker}. Japanese stock ticker must be 4 digits"
                )
        return v
...
class BatchPriceResponse(BaseModel):
    ...
    successful_count: int = Field(
        ..., description="Number of successful price retrievals"
    )

...
class WatchlistStockBase(BaseModel):
    ...
    stop_loss_price: Optional[Decimal] = Field(None, description="User's stop loss price")
    alert_price_above: Optional[Decimal] = Field(None, description="Alert when price goes above")
    alert_price_below: Optional[Decimal] = Field(None, description="Alert when price goes below")
...
class WatchlistStockWithPrice(WatchlistStock):
    ...
    price_change_percent: Optional[float] = Field(None, description="Price change percentage")
    volume_today: Optional[int] = Field(None, description="Today's trading volume")
    last_updated: Optional[datetime] = Field(None, description="Price data last updated")
...
class WatchlistAlertBase(BaseModel):
    ...
    current_value: Decimal = Field(..., description="Current value when alert triggered")
...
class WatchlistPerformance(BaseModel):
    ...
    best_performer: Optional[Dict[str, Any]] = Field(None, description="Best performing stock")
    worst_performer: Optional[Dict[str, Any]] = Field(None, description="Worst performing stock")
...
class WatchlistStatistics(BaseModel):
    ...
    total_stocks_watched: int = Field(..., description="Total stocks across all watchlists")
    most_watched_sectors: List[Dict[str, Any]] = Field(..., description="Most watched sectors")
    most_watched_stocks: List[Dict[str, Any]] = Field(..., description="Most watched stocks")
    average_watchlist_size: float = Field(..., description="Average stocks per watchlist")
    alerts_triggered_today: int = Field(..., description="Alerts triggered today")
    alerts_triggered_this_week: int = Field(..., description="Alerts triggered this week")
...
class WatchlistExportRequest(BaseModel):
    ...
    watchlist_ids: List[UUID] = Field(..., min_items=1, max_items=50, description="Watchlist IDs to export")

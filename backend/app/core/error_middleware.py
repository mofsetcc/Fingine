...
class ErrorHandlingMiddleware(BaseHTTPMiddleware):
    ...
    async def _handle_kessan_exception(
        self, exception: KessanException, request: Request, request_id: str
    ) -> JSONResponse:
        ...
        if self.enable_graceful_degradation:
            response_data = await self._apply_graceful_degradation(
                exception, request, response_data
            )
    ...
    async def _handle_http_exception(
        ...
        self.error_stats.record_error(
            kessan_exception.category, kessan_exception.severity
        )
        ...
        return JSONResponse(
            ...
            headers={
                ...
                "X-Error-ID": kessan_exception.error_id,
            },
        )
    ...
    async def _handle_unexpected_exception(
        ...
        try:
            ...
            logger.error(
                "Failed to send alert for unexpected error", error=str(alert_error)
            )
...
class GracefulDegradationService:
    ...
    async def get_cached_stock_data(ticker: str) -> Dict[str, Any]:
        ...
        return {"data": None, "is_cached": False, "message": "No data available"}
    ...
    def get_error_response_template(error_category: ErrorCategory) -> Dict[str, Any]:
        ...
        return templates.get(
            error_category, {"fallback_available": False, "retry_recommended": True}
        )

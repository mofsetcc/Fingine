...
class AlertManager:
    ...
    def __init__(self):
        ...
        self.pagerduty_integration_key = getattr(settings, 'PAGERDUTY_INTEGRATION_KEY', None)
    ...
    async def send_alert(
        ...
        if not await self.rate_limiter.should_alert(alert_key, exception.severity):
            logger.info("Alert rate limited", alert_key=alert_key, severity=exception.severity.value)
            return
        ...
        if self.pagerduty_integration_key and exception.severity == ErrorSeverity.CRITICAL:
            tasks.append(self._send_pagerduty_alert(alert_data))
        
        if tasks:
            try:
                await asyncio.gather(*tasks, return_exceptions=True)
                logger.info("Alerts sent", error_id=exception.error_id, alert_count=len(tasks))
            except Exception as e:
                logger.error("Failed to send alerts", error=str(e), error_id=exception.error_id)
    ...
    async def _send_slack_alert(self, alert_data: Dict[str, Any]) -> None:
        ...
        try:
            ...
            if alert_data.get("context"):
                context_text = "\n".join([f"• {k}: {v}" for k, v in alert_data["context"].items()])
                slack_message["attachments"][0]["fields"].append({
                    "title": "Context",
            ...
            async with aiohttp.ClientSession() as session:
                async with session.post(
                    ...
                    if response.status != 200:
                        ...
                        logger.info("Slack alert sent successfully", error_id=alert_data["error_id"])
        
        except Exception as e:
            logger.error("Exception sending Slack alert", error=str(e), error_id=alert_data["error_id"])
            ...
            async with aiohttp.ClientSession() as session:
                async with session.post(
                    ...
                    if response.status != 202:
                        ...
                        logger.info("PagerDuty alert sent successfully", error_id=alert_data["error_id"])
        
        except Exception as e:
            logger.error("Exception sending PagerDuty alert", error=str(e), error_id=alert_data["error_id"])
...
class SystemHealthAlerter:
    ...
    async def alert_database_connection_failure(self, error_details: Dict[str, Any]) -> None:
    ...
    async def alert_cache_connection_failure(self, error_details: Dict[str, Any]) -> None:
        """Alert for cache connection failures."""
        from app.core.exceptions import CacheConnectionException
        
        exception = CacheConnectionException(
            context=error_details,
            should_alert=True
        )
        await self.alert_manager.send_alert(exception, error_details)
    ...
    async def alert_budget_exceeded(self, budget_type: str, context: Optional[Dict[str, Any]] = None) -> None:
    ...
    async def alert_high_error_rate(
        ...
        from app.core.exceptions import KessanException, ErrorCategory, ErrorSeverity

...
class QuotaEnforcementMiddleware:
    ...
    def _should_enforce_quota(self, request: Request) -> bool:
        ...
        if method == "GET":
            if path.startswith("/api/v1/stocks/") and len(path.split("/")) == 5:  # /api/v1/stocks/{ticker}
                return True
            if path.startswith("/api/v1/analysis/") and len(path.split("/")) == 5:  # /api/v1/analysis/{ticker}
                return True
            if path.startswith("/api/v1/news/stock/") and len(path.split("/")) == 6:  # /api/v1/news/stock/{ticker}
                return True
            if path.startswith("/api/v1/sentiment/") and len(path.split("/")) == 5:  # /api/v1/sentiment/{ticker}
                return True

...
class PerformanceAlertManager:
    ...
    async def check_alert(self, alert_state: AlertState, current_metrics: Dict):
        ...
        cutoff_time = datetime.utcnow() - timedelta(minutes=alert.evaluation_window_minutes)
        ...
        should_trigger = await self.evaluate_condition(alert, alert_state, current_value)
    ...
    async def evaluate_condition(self, alert: PerformanceAlert, alert_state: AlertState, current_value: float) -> bool:
        ...
        if alert.condition == AlertCondition.THRESHOLD_ABOVE:
            ...
            previous_avg = sum(alert_state.historical_values[:-1]) / len(alert_state.historical_values[:-1])
            if previous_avg == 0:
                return False
    ...
    def get_alert_status(self) -> Dict[str, Dict]:
        ...
        for alert_name, alert_state in self.alert_states.items():
            status[alert_name] = {
                ...
                "last_triggered": alert_state.last_triggered.isoformat() if alert_state.last_triggered else None,
                "last_resolved": alert_state.last_resolved.isoformat() if alert_state.last_resolved else None,
...
def add_custom_threshold_alert(
    ...
    alert = PerformanceAlert(
        ...
        condition=AlertCondition.THRESHOLD_ABOVE if above else AlertCondition.THRESHOLD_BELOW,
        threshold=threshold,
        severity=severity,
        description=description or f"Metric {metric_name} {'above' if above else 'below'} {threshold}",
...
def add_custom_percentage_change_alert(
    ...
    alert = PerformanceAlert(
        ...
        severity=severity,
        description=description or f"Metric {metric_name} changed by {percentage_threshold}%",

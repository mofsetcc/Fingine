...
class YahooFinanceJapanAdapter(StockPriceAdapter):
    ...
    def __init__(
        ...
        self.delay_minutes = self.config.get(
            "delay_minutes", 15
    ...
    async def _make_request(
        ...
        for attempt in range(self.max_retries + 1):
            try:
                ...
                async with session.get(url, params=params) as response:
                    ...
                    if response.status == 200:
                        ...
                        if attempt < self.max_retries:
                            logger.warning(
                                f"Yahoo Finance server error (attempt {attempt + 1}): {response.status}"
                            )
                ...
                if attempt < self.max_retries:
                    logger.warning(
                        f"Yahoo Finance connection error (attempt {attempt + 1}): {e}"
                    )
                    await asyncio.sleep(self.retry_delay * (attempt + 1))
                    continue
                else:
    ...
    def _parse_chart_data(self, data: Dict[str, Any], symbol: str) -> Dict[str, Any]:
        ...
        try:
            ...
            if regular_market_time:
                trading_day = datetime.fromtimestamp(regular_market_time).strftime(
                    "%Y-%m-%d"
                )
            else:
            ...
            result_data = {
                ...
                "market_status": "open"
                if meta.get("marketState") == "REGULAR"
                else "closed",
        ...
        try:
            ...
            for i, timestamp in enumerate(timestamps):
                try:
                    ...
                    open_price = (
                        opens[i] if i < len(opens) and opens[i] is not None else 0
                    )
                    high_price = (
                        highs[i] if i < len(highs) and highs[i] is not None else 0
                    )
                    low_price = lows[i] if i < len(lows) and lows[i] is not None else 0
                    close_price = (
                        closes[i] if i < len(closes) and closes[i] is not None else 0
                    )
                    volume = (

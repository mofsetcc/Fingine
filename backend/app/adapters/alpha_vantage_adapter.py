...
class AlphaVantageAdapter(StockPriceAdapter):
    ...
    async def _make_request(self, params: Dict[str, str]) -> Dict[str, Any]:
        ...
        for attempt in range(self.max_retries + 1):
            try:
                logger.debug(
                    f"Making Alpha Vantage API request: {params.get('function', 'unknown')}"
                )
                ...
                async with session.get(url) as response:
                    ...
                    if response.status == 200:
                        ...
                        if attempt < self.max_retries:
                            logger.warning(
                                f"Alpha Vantage server error (attempt {attempt + 1}): {response.status}"
                            )
                            await asyncio.sleep(self.retry_delay * (attempt + 1))
                            continue
                        else:
                            raise DataSourceUnavailableError(
                                f"Alpha Vantage server error: {response.status}"
                            )
                ...
                if attempt < self.max_retries:
                    logger.warning(
                        f"Alpha Vantage connection error (attempt {attempt + 1}): {e}"
                    )
                    await asyncio.sleep(self.retry_delay * (attempt + 1))
                    continue
                else:
                    raise DataSourceUnavailableError(
                        f"Alpha Vantage connection error: {e}"
                    )
            ...
            if av_field in quote_data:
                ...
                if our_field in [
                    ...
                    try:
                        ...
                        logger.warning(
                            f"Invalid numeric value for {our_field}: {value}"
                        )
            ...
            try:
                ...
                record = {
                    ...
                    "adjusted_close": float(
                        price_data.get(
                            "5. adjusted close", price_data.get("4. close", 0)
                        )
                    ),
                }
    ...
    async def get_rate_limit_info(self) -> RateLimitInfo:
        ...
        return RateLimitInfo(
            ...
            requests_per_hour=self.requests_per_minute
            * 60,  # Not enforced by Alpha Vantage
    ...
    async def get_current_price(self, symbol: str) -> Dict[str, Any]:
        ...
        params = {"function": "GLOBAL_QUOTE", "symbol": normalized_symbol}

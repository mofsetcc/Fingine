...
class EDINETAdapter(FinancialDataAdapter):
    ...
    async def _make_request(self, endpoint: str, params: Optional[Dict[str, str]] = None) -> Dict[str, Any]:
        ...
        for attempt in range(self.max_retries + 1):
            try:
                ...
                async with session.get(url) as response:
                    ...
                    if response.status == 200:
                        ...
                        if 'application/json' in content_type:
                            ...
                            return {"text_data": text_data, "content_type": content_type}
                        ...
                        if attempt < self.max_retries:
                            logger.warning(f"EDINET server error (attempt {attempt + 1}): {response.status}")
                            await asyncio.sleep(self.retry_delay * (attempt + 1))
                            continue
                        else:
                            raise DataSourceUnavailableError(f"EDINET server error: {response.status}")
                ...
                if attempt < self.max_retries:
                    logger.warning(f"EDINET connection error (attempt {attempt + 1}): {e}")
                    await asyncio.sleep(self.retry_delay * (attempt + 1))
                    continue
                else:
    ...
    async def _search_documents(
        ...
        try:
            ...
            for result in data["results"]:
                ...
                # Parse dates
                submit_date = datetime.strptime(result["submitDateTime"], "%Y-%m-%d %H:%M")
                period_start = None
                period_end = None
    ...
    def _extract_facts(self, root: ET.Element, contexts: Dict[str, Dict[str, Any]]) -> List[Dict[str, Any]]:
        """Extract financial facts from XBRL."""
        facts = []
        
        # Find all elements that are not in the xbrli namespace (these are the facts)
        for elem in root:
            if elem.tag.startswith('{') and not elem.tag.startswith('{http://www.xbrl.org/2003/instance}'):
    ...
    def _categorize_facts(self, facts: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        for fact in facts:
            ...
            element_name = fact['name'].split('}')[-1] if '}' in fact['name'] else fact['name']
            ...
            if element_name in element_mapping:
                ...
                if element_name in ['NetSales', 'OperatingIncome', 'NetIncome', 'GrossProfit', 'SellingGeneralAndAdministrativeExpenses']:
                    financial_data['income_statement'][standard_name] = {
                        'value': value,
                        'period': fact['context']['period'],
                        'unit': fact['unit_ref']
                    }
                elif element_name in ['TotalAssets', 'TotalLiabilities', 'NetAssets', 'CashAndCashEquivalents', 'CurrentAssets', 'CurrentLiabilities']:
                    financial_data['balance_sheet'][standard_name] = {
                        'value': value,
                        'period': fact['context']['period'],
                        'unit': fact['unit_ref']
                    }
                elif element_name in ['CashFlowsFromOperatingActivities', 'CashFlowsFromInvestingActivities', 'CashFlowsFromFinancingActivities']:
                ...
                try:
                    ...
                    if statement_type in financial_data:
                        statement_data = {
                            ...
                            "fiscal_period": self._determine_fiscal_period(doc.period_start, doc.period_end),
    ...
    def _determine_fiscal_period(self, start_date: Optional[datetime], end_date: Optional[datetime]) -> str:
    ...
    async def get_company_overview(self, symbol: str) -> Dict[str, Any]:
        ...
        try:
            ...
            overview = {
                ...
                "fiscal_year_end": latest_doc.period_end.isoformat() if latest_doc.period_end else None,
                "source": "EDINET"
            }
            ...
            try:
                ...
                if "balance_sheet" in financial_data:
                    ...
                    if "shareholders_equity" in bs_data:
                        overview["shareholders_equity"] = bs_data["shareholders_equity"]["value"]

...
from .base import NewsAdapter, HealthStatus, HealthCheck, RateLimitInfo, CostInfo
...
class NewsDataAdapter(NewsAdapter):
    ...
    def __init__(self, name: str = "news_data_adapter", priority: int = 100, config: Optional[Dict[str, Any]] = None):
    ...
    async def _get_session(self) -> aiohttp.ClientSession:
        ...
        if self._session is None or self._session.closed:
            ...
            self._session = aiohttp.ClientSession(
                timeout=timeout,
                headers={
                    "User-Agent": "Project Kessan News Aggregator/1.0"
                }
            )
    ...
    async def health_check(self) -> HealthCheck:
        ...
        try:
            ...
            return HealthCheck(
                ...
                metadata={"sources_tested": len(self.rss_sources) + (1 if self.news_api_key else 0)}
            )
    ...
    async def _test_rss_feeds(self) -> None:
        ...
        if self.rss_sources:
            ...
            async with session.get(source["url"]) as response:
                if response.status != 200:
                    raise Exception(f"RSS feed {source['name']} returned status {response.status}")
    ...
    async def get_news(
        ...
        try:
            ...
            # Score relevance
            scored_articles = self._score_relevance(deduplicated_articles, symbol, keywords)
    ...
    async def _get_rss_articles(
        ...
        for source in self.rss_sources:
            task = self._process_rss_source(source, symbol, keywords, start_date, end_date)
            tasks.append(task)
    ...
    async def _process_rss_source(
        ...
        try:
            ...
            async with session.get(source["url"]) as response:
                if response.status != 200:
                    logger.warning(f"RSS source {source['name']} returned status {response.status}")
                    return []
                ...
                try:
                    ...
                    for item in items:
                        ...
                        if article and self._filter_article(article, symbol, keywords, start_date, end_date):
    ...
    def _normalize_news_api_article(self, article: Dict[str, Any]) -> Optional[Dict[str, Any]]:
    ...
    def _normalize_rss_article_xml(self, item: ET.Element, source: Dict[str, Any]) -> Optional[Dict[str, Any]]:
    ...
    def _score_relevance(
        ...
        for article in articles:
            ...
            try:
                article_date = datetime.fromisoformat(article.get("published_at", "").replace("Z", "+00:00"))
                hours_old = (datetime.utcnow() - article_date.replace(tzinfo=None)).total_seconds() / 3600
                if hours_old < 24:

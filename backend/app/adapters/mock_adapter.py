...
class MockStockPriceAdapter(StockPriceAdapter):
    ...
    async def _simulate_delay_and_failure(self) -> None:
        ...
        if self._should_fail or (
            self._failure_rate > 0 and random.random() < self._failure_rate
        ):
    ...
    async def search_symbols(self, query: str) -> List[Dict[str, Any]]:
        ...
        for symbol_data in mock_symbols:
            if (
                query_lower in symbol_data["symbol"].lower()
                or query_lower in symbol_data["name"].lower()
            ):
...
class MockFinancialDataAdapter(FinancialDataAdapter):
    ...
    async def health_check(self) -> HealthCheck:
        ...
        try:
            ...
            status = (
                HealthStatus.UNHEALTHY if self._should_fail else HealthStatus.HEALTHY
            )

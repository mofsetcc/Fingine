...
def setup_yahoo_finance_adapter(
    ...
    try:
        ...
        if register_adapter:
            ...
            logger.info(
                f"Registered Yahoo Finance adapter: {name} (priority: {priority})"
            )
...
async def test_yahoo_finance_connection(
    ...
    try:
        ...
        if health.status.value in ["healthy", "degraded"]:
            ...
            logger.warning(
                f"Yahoo Finance connection test failed: {health.error_message}"
            )
            return False
    ...
    try:
        ...
        config = create_yahoo_finance_config(
            ...
            requests_per_minute=int(
                os.getenv("YAHOO_FINANCE_REQUESTS_PER_MINUTE", "30")
            ),

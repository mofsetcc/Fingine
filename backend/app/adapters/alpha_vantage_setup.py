...
def setup_alpha_vantage_adapter(
    ...
    try:
        ...
        if register_adapter:
            ...
            logger.info(f"Registered Alpha Vantage adapter: {name} (priority: {priority})")
...
def validate_alpha_vantage_config(config: Dict[str, Any]) -> bool:
    ...
    numeric_fields = [
        "requests_per_minute", "requests_per_day", "timeout",
        "max_retries", "retry_delay", "cost_per_request", "monthly_budget"
    ]
    ...
    for field in numeric_fields:
        if field in config:
            ...
            if not isinstance(value, (int, float)) or value < 0:
                raise ValueError(f"Invalid value for {field}: must be non-negative number")
